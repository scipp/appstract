...
_formatter_providers = ProviderGroup()
...
_DEFAULT_MESSAGE_HEADERS = LogHeader(
...
AppCrafterDefaultHeader = NewType("AppCrafterDefaultHeader", LogHeader)


@_formatter_providers.provider
def provide_default_headers() -> AppCrafterDefaultHeader:
    return AppCrafterDefaultHeader(
        LogHeader(
            LogColumn("asctime", title="TIME", min_length=23),
            LogColumn("levelname", title="LEVEL", min_length=8),
            LogColumn("message", title=_DEFAULT_MESSAGE_HEADERS.format()),
            padding=(1, 1),
            sep="|",
        )
    )
...
AppCrafterFileFormatter = NewType("AppCrafterFileFormatter", Formatter)


@_formatter_providers.provider
def provide_file_formatter(
    log_header: AppCrafterDefaultHeader,
) -> AppCrafterFileFormatter:
    return AppCrafterFileFormatter(Formatter(log_header.fmt, style=log_header.style))


@_formatter_providers.provider
class AppCrafterStreamHighlighter(Highlighter):
